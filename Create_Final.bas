Attribute VB_Name = "Create_Final"
Option Explicit
...
For i_data = 3 To lrData
    If mData.Cells(i_data, 23).Value = ThisWorkbook.Sheets(3).Cells(k_report_list, 4).Value Then
    ' them cac thong tin chinh
        mReport.Range("Y8").Value = mData.Cells(i_data, 23).Value 'So bao cao
        mReport.Range("S11").Value = mData.Cells(i_data, 22).Value 'Ngay bao cao
