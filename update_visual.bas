Attribute VB_Name = "update_visual"
Option Explicit
    ...
    Dim welder1 As String
    Dim welder2 As String
...
'On Error GoTo c
    ...
    report_text = file_nguon.Range("J7").Value
    report_num = Trim(Right(report_text, Len(report_text) - 1))
'c:
End Sub
Sub DinhNghia_nguon_visual()
    spool_nguon = file_nguon.Cells(i_nguon, 10).Value
    ...
    welder1 = file_nguon.Cells(i_nguon, 8).Value
    welder2 = file_nguon.Cells(i_nguon, 9).Value
End Sub
Sub DinhNghia_data_visual()
    ...
    data_cell_rp = data_ws.Cells(j_data, 27).Value
    data_cell_date = data_ws.Cells(j_data, 26).Value
End Sub
...
Call DinhNghia_nguon_visual

    For j_data = 7 To lr_data
    Call DinhNghia_data_visual
   
        If spool_data = spool_nguon And joint_data = joint_nguon And _
        dwsheet_data = dwsheet_nguon And dw_data = dw_nguon Then
            data_ws.Cells(j_data, 26).Value = report_date
            data_ws.Cells(j_data, 27).Value = report_num
            data_ws.Cells(j_data, 24).Value = welder1
            data_ws.Cells(j_data, 25).Value = welder2
...
ActiveWorkbook.Sheets(1).Range("C1:C" & trung_dwsheet.Count).NumberFormat = "@"
    ...
    ActiveWorkbook.SaveAs ThisWorkbook.Path & "\" & "dulieuVisualtrung.xlsx"
    ActiveWorkbook.Close
